...
`,
  ...
  async rows() {
    ...
    }
    {
      ...
      rows.push(row(`diff: refetch, 3 changed, independently built (${ValueList.diff(vl, refetched).length} hunks)`, { ValueList: await timeSettled(() => ValueList.diff(vl, refetched), 2000), 'pointer scan': await timeSettled(() => scan(vl, refetched), 20) }));
    }
    return rows;
  },
};

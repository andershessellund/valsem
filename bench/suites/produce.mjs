import { timeSettled, timeHeld, row } from '../lib.mjs';
...
`,
  ...
  async rows() {
    ...
    const novel = () => ++seq;
    {
      ...
      const it = 2000;
      const v = await timeSettled(() => valsemProduce(vb, (d) => { d.arr[mid].value = novel(); }), it);
      setAutoFreeze(true);
      const ion = await timeSettled(() => immerProduce(ib, (d) => { d.arr[mid].value = novel(); }), 300);
      setAutoFreeze(false);
      const ioff = await timeSettled(() => immerProduce(ib2, (d) => { d.arr[mid].value = novel(); }), it);
      setAutoFreeze(true);
      const m = await timeSettled(() => mutativeCreate(mb, (d) => { d.arr[mid].value = novel(); }), it);
    ...
    }
    {
      ...
      rows.push(row('value-map 10k, one set, through the draft', {
        [C.v]: await timeSettled(() => valsemProduce(vb, (d) => { d.set('k5', novel()); }), it),
        [C.ion]: await timeSettled(() => immerProduce(ib, (d) => { d.set('k5', novel()); }), 300),
        [C.ioff]: null,
    ...
    }
    {
      ...
      const mb = nums.slice();
      const it = 2000;
      const v = await timeSettled(() => valsemProduce(vb, (d) => { d.set(mid, novel()); d.push(novel()); }), it);
      setAutoFreeze(false);
      const ioff = await timeSettled(() => immerProduce(ib, (d) => { d[mid] = novel(); d.push(novel()); }), it);
      setAutoFreeze(true);
    ...
    }
    {
      ...
      const it = 200_000;
      const v = await timeSettled(() => valsemProduce(vb, (d) => { d.x = novel(); }), it);
      setAutoFreeze(true);
      const ion = await timeSettled(() => immerProduce({ x: 1, y: 2, z: 3 }, (d) => { d.x = novel(); }), it);
      setAutoFreeze(false);
      const ioff = await timeSettled(() => immerProduce({ x: 1, y: 2, z: 3 }, (d) => { d.x = novel(); }), it);
      setAutoFreeze(true);
      const m = await timeSettled(() => mutativeCreate({ x: 1, y: 2, z: 3 }, (d) => { d.x = novel(); }), it);
    ...
    }
    {
      ...
      const held = [];
      const it = 2000;
      const v = await timeSettled((i) => { held[i % 10] = valsemProduce(vb, (d) => { d.arr[mid].value = i % 10; }); }, it);
      setAutoFreeze(true);
      const ion = await timeSettled((i) => { held[i % 10] = immerProduce(ib, (d) => { d.arr[mid].value = i % 10; }); }, 300);
      setAutoFreeze(false);
      const ioff = await timeSettled((i) => { held[i % 10] = immerProduce(ib2, (d) => { d.arr[mid].value = i % 10; }); }, it);
      setAutoFreeze(true);
      const m = await timeSettled((i) => { held[i % 10] = mutativeCreate(mb, (d) => { d.arr[mid].value = i % 10; }); }, it);

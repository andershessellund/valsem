...
`,
  ...
  async rows() {
    ...
    h.set({ items: big }, 1);
    rows.push(row('get, raw 50-record payload key', { HashMap: await timeSettled(() => h.get({ items: big.map((x) => ({ ...x })) }), 20_000), 'native Map': null }));
    const s = new HashMap();
    rows.push(row('set, a novel raw 2-key object per call (a pool entry each)', { HashMap: await timeSettled((i) => s.set({ table: 'x', id: i }, 1), 100_000), 'native Map': null }));
    return rows;
  },
};

...
`,
  ...
  rows() {
    ...
    // Three processes per column, alternating, best per row. Within a process
    // the worker already takes the best of several repetitions, but some rows
    // are bimodal BETWEEN processes (the same edit settles at 3.7 µs or at
    // 8 µs for a whole process, whichever column it is), and one process per
    // column would print that as a difference the switch does not make.
    const best = { on: {}, off: {} };

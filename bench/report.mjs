...
const runs = [load('node'), load('bun'), load('spidermonkey')].filter(Boolean);
...
out.push('');
out.push(
  'Every number below is produced by `pnpm bench`, which runs the suites in `bench/suites/` on Node (V8) and on Bun ' +
    '(JavaScriptCore), and the engine-level ones in the SpiderMonkey shell (Firefox\'s engine) where one is installed; ' +
    'it writes one JSON file per runtime and renders this file. Each section describes exactly what its suite measures. ' +
    `Cells show **${runs.map((r) => r.runtime.name).join(' / ')}**, or fewer where a section says which runtimes ran it. The rationale behind ` +
...
for (const id of suiteIds) {
  // Only the runtimes that ran this suite get a slot in its cells: a shell
  // runs two suites, and a trailing " / —" on every other row says nothing.
  const ran = runs.filter((r) => r.suites.some((s) => s.id === id));
  const per = ran.map((r) => r.suites.find((s) => s.id === id));
  ...
  const strays = per
    .map((s, i) => (s && s.commit !== undefined && s.commit !== ran[i].commit ? `${ran[i].runtime.name} at \`${s.commit}\` (${s.date})` : null))
    .filter(Boolean);
  if (strays.length !== 0) {
    out.push(`*Measured at another commit than the header's: ${strays.join(', ')}.*`);
    out.push('');
  }
  if (ran.length !== runs.length) {

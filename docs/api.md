...
| `DraftMap` / `DraftSet` / `DraftList` / `DraftOrderedMap` / `DraftOrderedSet` | class | Mutable draft twins of the collections, handed out inside `produce`; `get()` returns drafts (`Draft<V>`). A draft has its value's methods (`DraftList`: `get`/`set`/`insert`/`remove`/`splice`/`push`/`pop`/`forEach`, plus `shift`/`unshift`), except the ones that build a new value (`slice`, `concat`, the set algebra): call those on `current(draft)`. The ordered drafts add `indexOf`/`at`/`first`/`last`/`insertAt`, and their patches (`omap.*`, `oset.*`) replay the recipe's operations in order. |

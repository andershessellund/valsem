...
| `OrderedMap` / `OrderedSet` | class | Insertion-ordered persistent map and set with canonical instances: the value is the entry (member) *sequence*, so order is part of it. `get`/`has`/`set`/`delete`/`indexOf`/`at`/`first`/`last`/`insertAt` are O(log n) through per-key anchors; iteration is in order. `at` is `Array.prototype.at` (from the end when negative, `undefined` for no entry), and `first`/`last` answer `undefined` when empty. `keyList`/`valueList` are the canonical `ValueList`s of the sequence. `OrderedMap` implements `ReadonlyMap`; `OrderedSet` has the `ReadonlySet` read API (`ValueSet.from(s)` for the algebra). |
| `ValueList` / `ValueMap` / `ValueSet` / `InternedString` | class | Persistent collections with canonical instances, built with `empty()`, `from(iterable)` and, for the list and the sets, `of(...items)` (`ValueList`: O(log n) `insert`/`remove`/`splice`/`slice`/`concat`, `setMany`, `Array`'s functional reads `map`/`filter`/`reduce`/`some`/`every`/`find`/`findIndex` (the sets have the first five) and its copying reorders `toSorted`/`toReversed`, and `ValueList.diff(a, b)` in O(c log n); positions are checked: `get`/`set`/`remove` name an element in `[0, length)` and `get` returns `T` (`arr[i]` with its type made true), while `at` is `Array.prototype.at` (from the end when negative, `undefined` for no element), `insert` and `splice`'s start take `[0, length]`, `slice` clamps as `Array`'s does and `splice`'s count is clamped to what is there (a negative count throws), and a non-integer is a `RangeError`, not index 0); `ValueMap` implements `ReadonlyMap`; `ValueSet` has the `ReadonlySet` read API and the ES2025 set algebra taking any iterable and returning `ValueSet`s, merged at node level (cost ∝ where the operands differ). |
| `RawArray` | class | A raw response you take canonical slices from: `slice(a, b)` admits only that range, `get(i)` / `at(i)` one row (each element interned once, memoized per slot), so a refetch's unchanged rows come back `===`. Its own value is its identity, so it sits in canonical state as an opaque leaf. |
| `ValueDate` | class | An immutable, canonical timestamp — the value a `Date` stands for. `ValueDate.from(x)` takes what `new Date(x)` takes; `toDate()` returns a fresh mutable `Date`; `valueOf()` is the epoch; `toJSON()` matches `Date`. |
| `toJSON()` on every collection | method | What `JSON.stringify` sees: `ValueList`, `ValueSet`, `OrderedSet`, `HashSet` give an array of their elements; `ValueMap`, `OrderedMap`, `HashMap` an array of `[key, value]` pairs, the shape `from()` takes (keys are values, so there is no object form); the drafts inside a recipe what they hold at that moment. A fresh plain array per call. A view, not a wire format: `ValueMap`/`ValueSet` output order follows the per-process hash seed, so it is not stable across processes; the ordered collections' is. |
| `util.inspect`, `Symbol.toStringTag` on every collection | | `console.log(list)` prints `ValueList(2) [ 1, 2 ]` (Node's inspect hook; the drafts print what they hold, a revoked one `[revoked DraftList]`), and `Object.prototype.toString` gives `[object ValueList]`. |
...
| `castDraft(value)` | function | `value`, typed as `Draft<T>`, for assigning a value into a draft slot: `d.todos = castDraft(ValueList.of(a, b))`, `d.tags = castDraft(d.tags.union(more))`. Does nothing at runtime: a recipe may put a value wherever a draft is, but a collection slot is typed as its draft class, and TypeScript has no wider type for writing than for reading. The same name and job as in immer and mutative. |
| `draftOf(value)` | function | Inside a recipe: a detached draft of `value` — a second root in the same scope, for material the recipe brings in from elsewhere (another store, a signal read inside a computed). Edit it, then attach it anywhere or return it; it resolves to its canonical value where it landed, one instance across every place. Unattached, it is dropped; revoked with the recipe. Non-draftables and drafts of the running recipe return themselves. |
...
| `DraftMap` / `DraftSet` / `DraftList` / `DraftOrderedMap` / `DraftOrderedSet` | class | Mutable draft twins of the collections, handed out inside `produce`; `get()` and iteration (`for…of`, `forEach`, `values()`, `entries()`) return drafts (`Draft<V>`), keys and set members stay values. The functional reads do not draft (callbacks see values, `map`/`filter` return values), except that `DraftList.find` returns the hit drafted. A draft has every method of its value (`DraftList` adds `shift`/`unshift`): what edits, edits the draft in place; what does not (`slice`, `concat`, the set algebra, `keyList`/`valueList`) answers about the value the draft would be right now, as `current(draft)` would, and returns values. The ordered drafts add `indexOf`/`at`/`first`/`last`/`insertAt`, and their patches (`omap.*`, `oset.*`) replay the recipe's operations in order. |
...
| `skipChecks()` / `skipFreezing()` | function | The two one-way switches you own: stop verifying *canonical only* arguments (`fastEqual`); stop freezing canonical records and arrays (faster iteration in V8, mutations no longer caught). Neither reads the environment. See the [performance guide](/guide/performance). |
...
| `DRAFT_STATE` | The symbol under which a draft object exposes its `DraftState`: define it, non-enumerable, in your draft's constructor. It is how `stateOf`, `isDraft` and `current` recognise your draft. |
| `isImmutable(value)` | Whether `value` is frozen or canonical: the test for "assigned material that must be drafted on read-back, not handed out as it is". |
| `same(a, b)` | SameValueZero, the comparison the built-in drafts use to tell a write that changes nothing (`NaN` over `NaN`) from an edit. |
| `emitSeqOps`, `retractSeqPatches`, `seqTailProfile` | The sequence-patch helpers arrays and `ValueList` share, for list-like kinds. |
| `DraftState`, `Scope`, `Patch`, `PatchKinds`, `PatchPath`, `PatchRecorder`, `SeqOp` | Types (`Scope` is the type of `DraftState.scope`, one per running recipe). Extend `PatchKinds` by declaration merging to add your own patch kinds with exact narrowing. |

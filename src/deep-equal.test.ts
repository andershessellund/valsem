...
describe('mutable built-ins — tier-1 registration is contained', () => {
  // The escape hatch: an app MAY register an EQUALITY for a mutable built-in.
  // deepEqual then answers by content — and that is all an equality claims.
  // A hash would declare immutability, so `register` refuses one for these
  // types, and deepHash/intern keep rejecting them: no mutable instance can
  // reach a pool, a collection, or a HashMap key.
  //
  ...
  it('registering an equality for Date enables comparison but never hashing or interning', async () => {
    expect(() =>
      deepEqual.register(
        Date,
    ...
    ).toThrow(/Date cannot be registered with a hash/);
